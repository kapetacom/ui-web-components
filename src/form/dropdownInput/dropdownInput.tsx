...
interface DropdownInputProps {
    ...
    selectionCallback: (userSelection: string[]) => void
}
...
export class DropdownInput extends React.Component<DropdownInputProps> {
    ...
    private selectHandler(selection: string) {
        ...
        if (tempUserSelection.length > 0) {
            ...
            if (isSelected > -1) {
                ...
                this.props.selectionCallback(this.userSelection);
                return;
            }
        }
    ...
    render() {
        ...
        let classNameLabel = toClass({
            "label": true,
            "zoom-out": this.labelZoomOut || this.userInput.length > 0 || !!this.inputFocus,
            "focused": !!this.inputFocus,
            "required": !!this.required && !this.userSelection.length && !this.inputFocus
        });

        let classNameDropdown = toClass({
            "dropdown": true,
            "focused": !!this.inputFocus
        });
        ...
        return (
            <div className={classNameDropdown} >
                <span id={this.props.id} className={classNameLabel} >{this.uppercaseFirstLetter(this.props.label)}</span>
                {this.userInput.length > 0 && <span className={"user-suggestion"}>{this.inputSuggestion}</span>}
                ...
                {this.userSelection.length > 0 ? <span className="selected-number">({this.userSelection.length})</span> : null}
                {
                    this.required && !this.userSelection.length && !this.inputFocus ?
                        <span className={"message required"}><sup>*</sup>Select at least one option</span> : null
                }
                {
                    (!this.inputFocus && this.props.message.length > 0 && this.userSelection.length) || !this.required ?
                        <span className={"message"} > {this.uppercaseFirstLetter(this.props.message)}</span> : null
                }
